...
export async function POST(request:Request){
  ...
  const {error}=await supabase.from('assessments').insert({user_id:user.id,edition_key:'conditioner-edition-v1',status:'completed',answers:parsed.data,customer_result:result,method_version:'hair-need-v1.0',completed_at:new Date().toISOString()})
